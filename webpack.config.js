    ...
    JSDOM.fromFile(filePath, {}).then(
      ({ window }) =>
        new Promise((resolve, reject) => {
          const scriptPaths = Array.from(
            ...
            .map((scriptPath) =>
              path.parse(
                path.join(
                  JS_INPUT_DIR,
                  path.relative(JS_OUTPUT_DIR, scriptPath)
                )
              )
            )
            .flatMap((pathObject) => {
          ...
          glob("{" + scriptPaths.join(",") + "}", (err, files) => {
...
module.exports = [
  async (env, argv) => ({
    ...
    entry: (await getScriptsAsync()).reduce((acc, item) => {
      const otherPath = path.parse(
        path.relative(
          ".",
          path.join(JS_OUTPUT_DIR, path.relative(JS_INPUT_DIR, item))
        )
      );
  ...
  async (env, argv) => ({
    ...
    entry: getStyles().reduce((acc, item) => {
      const otherPath = path.parse(
        path.relative(
          ".",
          path.join(CSS_OUTPUT_DIR, path.relative(CSS_INPUT_DIR, item))
        )
      );
      const key = path.format({
        dir: otherPath.dir,
